...
# from youtube_transcript_api import YouTubeTranscriptApi # Removed
...
rapidapi_key = os.getenv('RAPIDAPI_KEY') or st.secrets.get('RAPIDAPI_KEY', '')  # Load RapidAPI key
...
">
    ...
    <span style="color: rgba(255, 255, 255, 0.9);">
        For YouTube videos, ensure the video has available captions/transcriptions. 
        You will need to manually provide the language code (e.g., 'en' for English).
    </span>
</div>
""", unsafe_allow_html=True)
...
selected_lang = None  # Initialize selected_lang

if generic_url and "youtube.com" in generic_url and validators.url(generic_url):
    st.markdown("### 🌍 Specify Transcript Language")
    selected_lang = st.text_input(
        "Enter the language code for video transcript (e.g., 'en' for English):",
        value='en',
        help="Provide the language code of the video transcript you want to summarize. The tool cannot automatically list available languages with the current YouTube API."
    )
    st.info(f"📹 YouTube video detected. Attempting to fetch transcript in '{selected_lang}' (if available).")
...
if st.button("✨ Generate Summary", type="primary"):
    if not gemini_api_key.strip():
    ...
    elif not rapidapi_key.strip() and "youtube.com" in generic_url:
        st.error(
            "🔑 Please provide your RapidAPI key for YouTube transcript fetching in environment variables or Streamlit secrets.")
        ...
        try:
            ...
            with st.spinner("🚀 Processing your content..."):
                ...
                if "youtube.com" in generic_url:
                    if not selected_lang:
                        st.warning("⚠️ Please provide a language code for the transcript first.")
                        st.stop()

                    progress_bar.progress(40)
                    status_text.text(f"📺 Extracting YouTube transcript in '{selected_lang}' via RapidAPI...")

                    video_id = generic_url.split("=")[1]  # Extract video_id
                    ...
                    if isinstance(transcript_data, dict) and 'content' in transcript_data:
                        ...
                        st.error("❌ Unexpected transcript format returned by RapidAPI.")
                        st.stop()
            ...
            st.error(
                f"❌ Error fetching YouTube transcript from RapidAPI: {str(req_e)}. Please check your RapidAPI key and the video ID.")
            st.markdown("""
            <div style="margin-top: 1rem;">
                <details>
                    <summary style="color: #ff6b6b;">🔧 Troubleshooting Tips</summary>
                    <ul style="color: rgba(255, 255, 255, 0.8); margin-top: 0.5rem;">
                        <li>Ensure your <strong style="color: #ffc107;">RapidAPI Key</strong> is correct and has access to the YouTube Transcripts API.</li>
                        <li>Verify the YouTube URL is valid and the video is publicly accessible.</li>
                        <li>Check if the specified language for the transcript is actually available for the video.</li>
                    </ul>
                </details>
            </div>
            """, unsafe_allow_html=True)
